(function (root, factory) {
    if (typeof define === 'function' && define.amd) {
        define ('three.OBJLoader2', ['three'], factory);
    }
    ...
    THREE.OBJLoader2 = (function () {
        ...
        OBJLoader2.prototype.run = function (prepData, workerSupportExternal) {
            ...
            var onMaterialsLoaded = function (materials) {
                ...
                if (Validator.isValid (available.obj.content)) {
                    ...
                    scope.load (available.obj.name, scope.callbacks.onLoad, null, null, scope.callbacks.onMeshAlter,
                                prepData.useAsync);
        ...
        OBJLoader2.prototype.parseAsync = function (content, onLoad) {
            ...
            this.workerSupport = Validator.verifyInput (this.workerSupport,
        ...
        var Parser = (function () {
            ...
            Parser.prototype.setCallbackBuilder = function (callbackBuilder) {
                ...
                if (!Validator.isValid (this.callbackBuilder)) {
            ...
            Parser.prototype.processLine = function (buffer, bufferPointer, slashSpacePattern, slashSpacePatternPointer,
                ...
                switch (buffer[0]) {
                    ...
                    case Consts.LINE_F:
                        this.rawMesh.processFaces (buffer, bufferPointer,
                                                   countSlashes (slashSpacePattern, slashSpacePatternPointer));
                        break;

                    case Consts.LINE_L:
                        this.rawMesh.processLines (buffer, bufferPointer,
                                                   countSlashes (slashSpacePattern, slashSpacePatternPointer));
                        break;
                ...
                if (Validator.isValid (result)) {
                    ...
                    if (this.logger.isDebug ()) {
                        this.logger.logDebug (
                            this.createRawMeshReport (this.rawMesh, this.inputObjectCount));
                    }
                    this.inputObjectCount++;

                    this.buildMesh (result, currentByte);
                    var progressBytesPercent = currentByte / this.totalBytes;
                    this.callbackProgress ('Completed [o: ' + this.rawMesh.objectName + ' g:' + this.rawMesh.groupName + '] Total progress: ' + (progressBytesPercent * 100).toFixed (
                        2) + '%', progressBytesPercent);
            ...
            Parser.prototype.buildMesh = function (result, currentByte) {
                ...
                for (var oodIndex in rawObjectDescriptions) {
                    ...
                    if (this.logger.isDebug ()) {
                        var materialIndexLine = Validator.isValid (
        ...
        var RawMesh = (function () {
            ...
            RawMesh.prototype.pushUsemtl = function (mtlName) {
                if (this.activeMtlName === mtlName || !Validator.isValid (mtlName)) {
                    return;
                }
            ...
            RawMesh.prototype.verifyIndex = function () {
                ...
                if (!Validator.isValid (this.subGroupInUse)) {
                    ...
                    this.subGroupInUse = new RawMeshSubGroup (this.objectName, this.groupName, this.activeMtlName,
        ...
        OBJLoader2.prototype._checkFiles = function (resources) {
            ...
            for (var index in resources) {
                ...
                if (!Validator.isValid (resource.name)) {
                    continue;
                }
        ...
        OBJLoader2.prototype._loadMtl = function (resource, callbackOnLoad, crossOrigin) {
            ...
            // fast-fail
            if (!Validator.isValid (resource) || (!Validator.isValid (resource.content) && !Validator.isValid (
                    resource.url))) {
