(function (root, factory) {
    if (typeof define === 'function' && define.amd) {
        define ('three.SEA3DLegacy', ['three'], factory);
    }
    ...
    THREE.SEA3D.prototype.flipScaleMatrix = function () {
        ...
        return function (local, rotate, parent, parentRotate) {
            ...
            if (parent) {
                local.multiplyMatrices (parent, local);
            }
    ...
    THREE.SEA3D.prototype.updateTransform = function () {
        ...
        return function (obj3d, sea) {
            ...
            if (this.isLegacy (sea)) {
                ...
                if (sea.transform) {
                    buf1.fromArray (sea.transform);
                } else {
                    buf1.makeTranslation (sea.position.x, sea.position.y, sea.position.z);
                }
                ...
                for (var j = 0; j < numJoints; j++) {
                    ...
                    for (var frame = start; frame < end; frame++) {
                        ...
                        mtx_local.makeRotationFromQuaternion (
                            THREE.SEA3D.QUABUF.set (raw[idx + 3], raw[idx + 4], raw[idx + 5], raw[idx + 6]));
    ...
    THREE.SEA3D.prototype.readGeometryBuffer = function (sea) {
        ...
        if (this.isLegacy (sea)) {
            ...
            if (sea.jointPerVertex > 4) {
                this.compressJoints (sea);
            } else if (sea.jointPerVertex < 4) {
                this.expandJoints (sea);
            }
